...
var Twinkle = function () {
    ...
    Twinkle.prototype = {
        ...
        initSymbols: function initSymbols(color, radius, blur) {
        ...
        render: function render(ctx) {
            ...
            for (i = 0, len = particles.length; i < len; i++) {
                ...
                if (p.x + radius < fieldLeft || p.x - radius > fieldRight || p.y + radius < fieldTop || p.y - radius > fieldBottom || p.scale <= 0) {
                    this._pool.push(p);
        ...
        _createParticle: function _createParticle(magMin, magMax, scaleMax) {
            var mag = magMin + (magMax - magMin) * Math.random(),
                angle = Math.PI * 2 * Math.random(),
                p = this._pool.length ? this._pool.shift() : new Particle();

            p.init(this.mouse.x, this.mouse.y, mag * Math.cos(angle), mag * Math.sin(angle), scaleMax * Math.random(), Math.PI * 2 * Math.random());
    ...
    var parseColor = function () {
        ...
        return function (str) {
            ...
            if (match = str.match(RE_RGB) || str.match(RE_RGBA)) {
                return ['rgba', parseInt(match[1], 10), parseInt(match[2], 10), parseInt(match[3], 10), parseFloat(match.length === 4 ? 1 : match[4])];
            }

            // HSL(A)
            if (match = str.match(RE_HSL) || str.match(RE_HSLA)) {
                return ['hsla', parseFloat(match[1]), parseFloat(match[2]), parseFloat(match[3]), parseFloat(match.length === 4 ? 1 : match[4])];
            }
            ...
            if (match = str.match(RE_HEX)) {
                return ['rgba', parseInt(match[1], 16), parseInt(match[2], 16), parseInt(match[3], 16), 1];
            }
    ...
    function colorToString(type, v0, v1, v2, a) {
        ...
        if (type === 'hsla') return 'hsla(' + v0 + ',' + v1 + '%,' + v2 + '%,' + a + ')';else return '';
    }
...
(function () {
    ...
    var canvas, context, canvasBound, grad, twinkle, gui, timeoutId;
